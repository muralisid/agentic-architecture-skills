...
> **In plain terms.**
>
> This page covers how agents are identified and what stops them doing things they should not. The rule is simple. Every agent has its own named account, with only the access its current task needs. A fixed rules engine, not the AI itself, says yes or no to every action that matters. AI-based filters can warn, but they are not reliable enough to be the lock on the door. The one thing to remember: attackers will sometimes trick an agent. Build so that a tricked agent can do very little harm.
