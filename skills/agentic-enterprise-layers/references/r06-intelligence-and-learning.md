...
> **In plain terms.**
>
> This page is about how agents get better over time without getting worse in ways nobody notices. The people who know the work write the tests, and a model does most of the grading with humans checking a sample. Any new rule has to prove itself against past failures before it counts. The thing to remember: improvement is a controlled release process with a way back, not something the agent does to itself.
...
**Figure: The governed flywheel.** Production traces and eval datasets share a data layer; promotion is gated on counterexample survival and eval regression, and promoted rules land outside the model.

The optimizer and the evaluator stay decoupled; every promoted artifact has a demotion path.
